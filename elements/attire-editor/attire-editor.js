...
Polymer({
  ...
  _getUniqueName: function _getUniqueName() {
    ...
    var _loop = function _loop() {
      var input = prompt(_this3.localize("attire-editor-add-new-attires-message") + (withExistsWarning ? _this3.localize("attire-editor-add-new-attires-message-warning") : ""));
      if (!input) return {
          v: void 0
        };
  ...
  _sanitizeAttire: function _sanitizeAttire() {
    ...
    attire.rules = attire.rules.filter(function (it) {
      return _this5._isValid(it.when.red) && _this5._isValid(it.when.green) && _this5._isValid(it.when.blue) && _this5._isValid(it.when.black) && it.image !== null;
    }).map(function (it) {
